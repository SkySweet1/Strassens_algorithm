...
THRESHOLD это порог вхождения матрицы, при котором еще умножение будет происходить по алгоритму Штрассена
*/

#define MIN_MAT_SIZE 1
#define MAX_MAT_SIZE 1024
/*
Выводим здесь #define, чтобы можно было менять при необходимости
*/
...
typedef struct {} Matrix; чтобы не писать каждый раз struct Matrix, а просто писать Matrix
...
long long используеться для безумно больших чисел
...
(long long)a * b                        Превращает тип int в тип long long, чтобы засунуть в long long result
...
Без очистки памяти, так как она будет утекать и не освобождаться, что приведет к падению программы 
*/
